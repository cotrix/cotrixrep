...
public class VersioningTest {
	...
	public void attributesCanBeCopied() {
		...
		Attribute.Private a = (Attribute.Private) attr("id").name(name).value(value).in(language).build();
	...
	public void containersCanBeCopied() {
		...
		//little trick: we give object the same id that will be generated by our test generator
		//we can then use equality
		Attribute.Private a = (Attribute.Private) attr("id1").name(name).value(value).build();
		Attribute.Private a2 = (Attribute.Private) attr("id2").name(name2).value(value2).build();
	...
	public void codesCanBeCopied() {
		...
		//little trick: we give object the same id that will be generated by our test generator
		//we can then use equality
		Attribute a = attr("id1").name(name).value(value).build();
		Attribute a2 = attr("id2").name(name2).value(value2).build();
				
		Code.Private code = (Code.Private) code("id3").name(name).attributes(a,a2).build();
