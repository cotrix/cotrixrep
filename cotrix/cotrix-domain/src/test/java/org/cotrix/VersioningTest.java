...
public class VersioningTest {
	...
	public void copiesDoNotPreserveDeltas() {
		...
		Attribute a = a().with(name).and(value).as(Change.NEW).build();
	...
	public void attributesCanBeCopied() {
		...
		Attribute a = a(generator.generateId()).with(name).and(value).in(language).build();
	...
	public void bagsCanBeCopied() {
		...
		//little trick: we give object the same id that will be generated by our test generator
		//we can then use equality
		Attribute a = a(generator.generateId()).with(name).and(value).build();
		Attribute a2 = a(generator.generateId()).with(name2).and(value2).build();
		...
		//copy is recursive
		assertNotSame(bag.get(name).get(0), clone.get(name).get(0));

	}
	
	@Test
	public void groupsCanBeCopied() {
		...
		BaseGroup<Attribute> group = group(a,a2);
		
		BaseGroup<Attribute> clone = group.copy(generator);
	...
	public void codesCanBeCopied() {
		...
		Code code = code(generator.generateId()).with(name).and(a,a2).build();
	...
	public void versioningRequiresIdentifier() {
		...
		Codelist list = codelist().with(name).version(version).build(); 
	...
	public void codelistsCanBeVersioned() {
		...
		Code code = code(generator.generateId()).with(name).build();
		
		Codelist list = codelist("1").with(name).with(code).version(version).build(); 
	...
	public void codebagsCanBeVersioned() {
		...
		Codelist list = codelist(generator.generateId()).with(name).build();
		
		Codebag bag = codebag("1").with(name).with(list).version(version).build(); 
	...
	public void versionsMustBeConsistent() {
		...
		Codelist list = codelist("1").with(name).version("2").build(); 
