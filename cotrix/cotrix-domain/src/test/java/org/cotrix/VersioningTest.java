...
public class VersioningTest {
	...
	public void copiesDoNotPreserveDeltas() {
		...
		Attribute a = attr().name(name).value(value).as(Change.NEW).build();
	...
	public void attributesCanBeCopied() {
		...
		Attribute a = attr(generator.generateId()).name(name).value(value).in(language).build();
	...
	public void versioningRequiresIdentifier() {
		...
		Codelist list = codelist().name(name).version(version).build(); 
	...
	public void codelistsCanBeVersioned() {
		...
		Code code = code(generator.generateId()).name(name).build();
		
		Codelist list = codelist("1").name(name).with(code).version(version).build(); 
	...
	public void codebagsCanBeVersioned() {
		...
		Codelist list = codelist(generator.generateId()).name(name).build();
		
		Codebag bag = codebag("1").name(name).with(list).version(version).build(); 
	...
	public void versionsMustBeConsistent() {
		...
		Codelist list = codelist("1").name(name).version("2").build(); 
