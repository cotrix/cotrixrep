...
public class AttributedTypeMS extends IdentifiedMS implements Attributed.State {

	private NamedStateContainer<Attribute.State> attributes = new NamedStateContainer.Default<Attribute.State>();

	public AttributedTypeMS() {
		attributes.add(timestamp(CREATION_TIME));
	}
	
	public AttributedTypeMS(String id,Status status) {
		super(id,status);
	}
	
	public AttributedTypeMS(Attributed.State other) {
