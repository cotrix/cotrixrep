...
public class CodelistPanelController implements Presenter {
	...
	private void loadCaches() {
		//FIXME tmp solution
		linkTypesCache.getItems(Nop.<Collection<UILinkDefinition>>getInstance());
		attributeTypesCache.getItems(Nop.<Collection<UIAttributeDefinition>>getInstance());
	}
				...
				if (isAboutOurCodelist(event)) {
					...
					linkTypesCache.getItems(new AsyncCallback<Collection<UILinkDefinition>>() {
						...
						public void onSuccess(Collection<UILinkDefinition> result) {
							...
							UILinkDefinition referencedLink = getOurLinkTypesReferTheModifiedItem(event, result);
			...
			public void onLinkRefreshed(final CodelistLinkRefreshedEvent event) {
				...
				linkTypesCache.getItems(new AsyncCallback<Collection<UILinkDefinition>>() {
					...
					public void onSuccess(Collection<UILinkDefinition> result) {
						...
						UILinkDefinition ourLinkType = ourLinkTypesReferTheModifiedCodelistLinkType(result, event.getCodelistId(), event.getLinkType());
